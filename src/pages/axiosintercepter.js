...
api.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (error.response && error.response.status === 401 && localStorage.getItem('refreshToken')) {
      try {
    ...
    if (!localStorage.getItem('accessToken') || (error.response && error.response.status === 401)) {
      localStorage.removeItem('accessToken');
      localStorage.removeItem('refreshToken');
      window.location.href = '/login'; // Redirect to login page
    }
